...
sub printGff {
    ...
    $attributes .= 'ID='    . $store->{'gene'}                     if (defined $store->{'gene'});
    ...
    $attributes  = "Parent=$store->{'gene'};$attributes" if $store->{'type'}=~/CDS/;
    $attributes .= ';';
    
    print "$seqId\tEMBL\t".$store->{'type'}."\t".$store->{'s'}."\t".$store->{'e'}."\t.\t".$store->{'strand'}."\t.\t$attributes\n";
    print "$seqId\tEMBL\tmRNA\t".$store->{'s'}."\t".$store->{'e'}."\t.\t".$store->{'strand'}."\t.\t$attributes\n" if($store->{'type'} =~ /CDS/);
